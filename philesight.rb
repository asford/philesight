...
class Philesight
	...
	def readdir(dir, skip, only_dirs)
		...
		begin
			Dir.foreach(dir) do |f|
				...
				if(f != "." && f != "..") then
					...
					if (skip.include?(f_full)) then
						STDERR.puts " skipping #{f_full}"
						next
					end
					...
					if(!stat.symlink?) then
						...
						if(stat.file?) then
							if(stat.nlink == 1) then
								size = File.size(f_full)
								size_file[f] = size
								size_total += size
							else
								if not @hardlink_list.include? stat.ino
									size = File.size(f_full)
									size_file[f] = size
									size_total += size
									@hardlink_list << stat.ino
									end
							end
						end

						if(stat.directory?) then
							size = readdir(f_full, skip, only_dirs) 
							if(size > 0) then
								size_dir[f] = size
								size_total += size
							end
		...
		# Store the directories and files in the database

		if only_dirs then

			children = size_dir.keys.sort
	...
	def index(path, skip, one_fs, only_dirs)
		if(one_fs) then
	...
	def filesize_readable(size)
		if(size > 1024*1024*1024*1024) then
			return "%.1f TiB" % (size / (1024.0*1024*1024*1024))
		elsif(size > 1024*1024*1024) then
			return "%.1f GiB" % (size / (1024.0*1024*1024))
		elsif(size > 1024*1024) then
			return "%.1f MiB" % (size / (1024.0*1024))
